...
class ContratosController extends Controller
{
    ...
    public function state($id)
    {
        ...
        // NOTA: el guard "Suspender al tener = No aplica" NO se aplica aquí porque
        // es una acción manual del operador desde la tabla de contratos (uno a uno).
        // El guard solo protege la suspensión automática (cron / análisis de cortes).
    ...
    public function state_oltcatv($id)
    {
        ...
        // NOTA: el guard "Suspender al tener = No aplica" NO se aplica aquí porque
        // es una acción manual del operador (corte TV uno a uno desde la tabla).
    ...
    public function state_lote($contratos, $state)
    {
        ...
        for ($i = 0; $i < count($contratos); $i++) {
            ...
            // NOTA: el guard "Suspender al tener = No aplica" NO se aplica aquí:
            // es una acción manual en lote del operador desde la tabla de contratos.
            // El guard solo protege la suspensión automática (cron / análisis de cortes).
                    ...
                    if ($API->connect($mikrotik->ip, $mikrotik->usuario, $mikrotik->clave)) {
                        switch ($state) {
                            ...
                            case 'enabled':
                                ...
                                $movimiento->modulo      = 5;
                                $movimiento->descripcion = '[STATE_LOTE] Búsqueda en morosos para IP ' . $contrato->ip . ': ' . count($ARRAYS) . ' entrada(s) encontrada(s).';
                                $movimiento->created_by  = Auth::user()->id;
                                $movimiento->empresa     = Auth::user()->empresa;
                                $movimiento->save();
                                ...
                                if (count($ARRAYS) > 0) {
                                    ...
                                    $movimiento->contrato    = $contrato->id;
                                    $movimiento->modulo      = 5;
                                    $movimiento->descripcion = '[STATE_LOTE] Intentando remover de morosos la IP ' . $contrato->ip . ' (' . count($idsToRemove) . ' entrada(s): ' . implode(', ', $idsToRemove) . ')';
                                    $movimiento->created_by  = Auth::user()->id;
                                    $movimiento->empresa     = Auth::user()->empresa;
                                    $movimiento->save();
                                    ...
                                    $descVerif = empty($verificacion)
                                        ? '[STATE_LOTE] Verificación exitosa: IP ' . $contrato->ip . ' ya no está en la lista de morosos.'
                                        : '[STATE_LOTE] ADVERTENCIA: IP ' . $contrato->ip . ' sigue en morosos (' . count($verificacion) . ' entrada(s) restantes).';
                                    ...
                                    $movimiento->contrato    = $contrato->id;
                                    $movimiento->modulo      = 5;
                                    $movimiento->descripcion = '[STATE_LOTE] IP ' . $contrato->ip . ' no se encontró en morosos al habilitar contrato.';
                                    $movimiento->created_by  = Auth::user()->id;
                                    $movimiento->empresa     = Auth::user()->empresa;
                                    $movimiento->save();
                                }
                                ...
                                if (isset($empresa->activeconn_secret) && $empresa->activeconn_secret == 1) {
                                    if ($contrato->conexion == 1 && $contrato->usuario != null) {
                                        ...
                                        if (count($ARRAYS) > 0) {
                                            ...
                                            $movimiento->contrato    = $contrato->id;
                                            $movimiento->modulo      = 5;
                                            $movimiento->descripcion = '[STATE_LOTE] Secret PPPoE "' . $contrato->usuario . '" habilitado en MikroTik.';
                                            $movimiento->created_by  = Auth::user()->id;
                                            $movimiento->empresa     = Auth::user()->empresa;
                                            $movimiento->save();
                                        }
                                ...
                                $movimiento->contrato    = $contrato->id;
                                $movimiento->modulo      = 5;
                                $movimiento->descripcion = '[STATE_LOTE] Resultado agregar a ips_autorizadas: ' . json_encode($resultAdd);
                                $movimiento->created_by  = Auth::user()->id;
                                $movimiento->empresa     = Auth::user()->empresa;
                                $movimiento->save();
            ...
            if ($descripcion == "") {
                $descripcion = '<i class="fas fa-check text-success"></i> <b>Cambio de Status</b> a ' . ($state == 'enabled' ? 'Habilitado' : 'Deshabilitado') . ' (Masivo)<br>';
            }
