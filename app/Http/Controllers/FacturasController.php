...
class FacturasController extends Controller
{
    ...
    public function indexold(Request $request)
    {
        ...
        $facturas = Factura::join('contactos as c', 'factura.cliente', '=', 'c.id')
          ...
          (if.cant*if.precio)-(if.precio*(if(if.desc,if.desc,0)/100)*if.cant)+(if.precio-(if.precio*(if(if.desc,if.desc,0)/100)))*(if.impuesto/100)*if.cant) as total'),
            ...
            ->where('factura.empresa', Auth::user()->empresa)->where('tipo', '!=', 5)->
        ...
        if ($request->has('search')) {
            ...
            if (is_numeric($request->input('search'))) {
                if ($filter != '') {
                    ...
                    $facturas          = $facturas->havingRaw(
                        'SUM((if.cant*if.precio)-(if.precio*(if(if.desc,if.desc,0)/100)*if.cant)+
                    (if.precio-(if.precio*(if(if.desc,if.desc,0)/100)))*(if.impuesto/100)*if.cant) > ?',
                        [$search]
                    );
                }
        ...
        if ($request->name_1) {
            ...
            $facturas = $facturas->where('factura.codigo', 'like', '%' . $request->name_1 . '%');
        }
        if ($request->name_2) {
            $busqueda = true;
        ...
        if ($request->name_3) {
            ...
            $facturas = $facturas->where('factura.fecha', date('Y-m-d', strtotime($request->name_3)));
        }
        if ($request->name_4) {
            $busqueda = true;
            $appends['name_4'] = $request->name_4;
            $facturas = $facturas->where('factura.vencimiento', date('Y-m-d', strtotime($request->name_4)));
        }
        if ($request->name_8) {
            $busqueda = true;
            $appends['name_8'] = $request->name_8;
            $facturas = $facturas->whereIn('factura.estatus', $request->name_8);
        }

        if ($request->name_6) {
            $busqueda = true;
            $appends['name_6'] = $request->name_6;
            $appends['name_6_simb'] = $request->name_6_simb;
            $facturas = $facturas->whereRaw(DB::raw('((Select SUM(pago) from ingresos_factura where factura=factura.id) + (Select if(SUM(valor), SUM(valor), 0) from ingresos_retenciones where factura=factura.id)) ' . $request->name_6_simb . ' ?'), [$request->name_6]);
        }


        if ($request->name_9) {
            $busqueda = true;
            $appends['name_9'] = $request->name_9;
            $facturas = $facturas->where('v.nombre', 'like', '%' . $request->name_9 . '%');
        }
        ...
        if ($request->name_10) {
            $busqueda = true;
            $appends['name_10'] = $request->name_10;
            $facturas = $facturas->where('cs.fecha_corte', $request->name_10);
        }

        if ($request->name_11) {
            $busqueda = true;
            $appends['name_11'] = $request->name_11;
            $facturas = $facturas->where('c.nit', 'like', '%' . $request->name_11 . '%');
        }

        if ($request->name_12) {
            $busqueda = true;
            $appends['name_12'] = $request->name_12;
            $facturas = $facturas->where('c.direccion', 'like', '%' . $request->name_12 . '%');
        }

        if ($request->name_13) {
            $busqueda = true;
            $appends['name_13'] = $request->name_13;
            $facturas = $facturas->where('cs1.server_configuration_id', $request->name_13);
        }

        if ($request->name_14) {
            $busqueda = true;
            $appends['name_14'] = $request->name_14;
            $facturas = $facturas->where('cs.ip', $request->name_14);
        }

        if ($request->name_15) {
            $busqueda = true;
            $appends['name_15'] = $request->name_15;
            $facturas = $facturas->where('cs.mac_address', $request->name_15);
        }
    ...
    public function indexNew(Request $request, $tipo)
    {
        ...
        $tipo = $tipo != '' ? ($tipo == 'cerradas' ? 'A' : 1) : null;
    ...
    */
    public function index_electronica()
    {
        ...
        $numeracionActual = NumeracionFactura::where('nomina', 0)->where('num_equivalente', 0)
            ->where('preferida', 1)
            ->where('estado', 1)
            ->where('tipo', 2)
            ->first();
    ...
    */
    ...
    public function create($producto = false, $cliente = false)
    {
        ...
        return view('facturas.create')->with(compact(
            'clientes',
            'tipo_documento',
            'inventario',
            'numeraciones',
    ...
    public function edit($id)
    {
        ...
        if ($factura) {
            if ($factura->estatus == 1) {
                ...
                return view('facturas.edit')->with(compact(
                    'clientes',
                    'inventario',
                    'vendedores',
                    'terminos',
    ...
    public function xmlFacturaVenta($id)
    {
        ...
        if ($ValidateInvoices > 0) {
            while (
                Factura::where('empresa', $empresaId)->where('codigo', $FacturaVenta->codigo)->where('id', '!=', $FacturaVenta->id)->count() > 0
            ) {
    ...
    public function facturasWhatsappIndex(Request $request)
    {
        ...
        $totalFaltantes = Factura::join('contracts as c', 'c.id', '=', 'factura.contrato_id')
        ...
        $sinTelefono = Factura::join('contracts as c', 'c.id', '=', 'factura.contrato_id')
