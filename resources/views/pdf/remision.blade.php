...
@section('content')
    <style type="text/css">
        ...
        **/#watermark { 
        ...
        .smalltd{
            font-size: 12px;line-height: 12px; padding-right: 2px;
        }
    ...
    margin-top: 2%;">
            <p class="medium text-center">Remisión</p>
            <h4 class=" text-center">No. #{{$remision->nro}}</h4>
    ...
    <div style="">
        <table border="1" class="titulo">
            <tr>
                ...
                <td colspan="3" style="border-top: 2px solid #ccc;">{{$remision->cliente()->nombre}} {{$remision->cliente()->apellidos()}}</td>
                <th width="22%" class="center" style="font-size: 8px"><b>FECHA DE EXPEDICIÓN (DD/MM/AA)</b></th>
            </tr>
            <tr>
                <th class="right smalltd">DIRECCIÓN</th>
                <td colspan="3">{{$remision->cliente()->direccion}}</td>
                <td class="center" style="    border-right: 2px solid #ccc;">{{date('d/m/Y', strtotime($remision->fecha))}}</td>
            </tr>
            <tr>
                <th class="right smalltd">CIUDAD</th>
                <td colspan="">{{$remision->cliente()->municipio()->nombre}}</td>
                <th class="right" style="padding-right: 2px;">TIPO DE PAGO</th>
            ...
            <tr>
                ...
                <td style="border-bottom: 2px solid #ccc;">{{$remision->cliente()->telefono1}}</td>
                <th class="right" style="padding-right: 2px;">{{$remision->cliente()->tip_iden('mini')}}</th>
                <td style="border-bottom: 2px solid #ccc;" >{{$remision->cliente()->nit}}</td>
                <td class="center" style="border-right: 2px solid #ccc; border-bottom: 2px solid #ccc;">{{date('d/m/Y', strtotime($remision->vencimiento))}}</td>
            </tr>
    ...
    <div style="margin-top: 2%;">
        <table border="0" class="desgloce" >
             ...
             <tbody>
                ...
                @php $cont++; @endphp
                    <tr>
                        ...
                        <td class="left padding-left border_left @if($cont==$itemscount && $cont>6) border_bottom @endif">{{$item->producto(false)->ref}}</td>
            ...
            <tfoot>
                <tr class="foot"> 
                    <th colspan="6" class="smalltd"></th>
                </tr>
                <tr class="foot">
                    <td colspan="4" class="smalltd">{{$remision->notas}}</td>
                    <td class="right">SubTotal</td>
                    <td class="right padding-right">{{Auth::user()->empresa()->moneda}}{{App\Funcion::Parsear($remision->total()->subtotal)}}</td>
                </tr>
                @if($remision->total()->descuento>0)
                    <tr class="foot">
                        <td colspan="4" class="smalltd"></td>
                        <td class="right">Descuento</td>
                        <td class="right padding-right">{{Auth::user()->empresa()->moneda}}{{App\Funcion::Parsear($remision->total()->descuento)}} </td>
                    </tr>
                    <tr class="foot">
                        <td colspan="4" class="smalltd"></td>
                        <td class="right">SubTotal</td>
                        <td class="right padding-right">{{Auth::user()->empresa()->moneda}}{{App\Funcion::Parsear($remision->total()->subsub)}}</td>
                    </tr>
                @endif
                ...
                <tr class="foot">
                    ...
                    <th class="right padding-right">{{Auth::user()->empresa()->moneda}}{{App\Funcion::Parsear($remision->total()->total)}} </th>
                </tr>
            </tfoot>
    ...
    <div style="width: 70%; margin-top: 1%">
        <p style="text-align: justify;" class="small">{{$remision->term_cond}}</p>
        <div style="padding-top: 8%; text-align: center;">
            <div style="display: inline-block; width: 45%; border-top: 1px solid #000;     margin-right: 10%;">
                <p class="small"> ELABORADO POR: {{$remision->vendedor()}}</p>
            </div>
    ...
    <div id="watermark">{{$remision->estatus==2?'ANULADA':''}}</div>
@endsection
