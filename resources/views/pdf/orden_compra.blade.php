...
@section('content')
    ...
    margin-top: 2%; margin-left: 15px;">
            <p class="small">Orden de compra</p>
            <h4 style="text-align: left; ">No. #{{$orden->orden_nro}}</h4>
    ...
    <div style="">
        <table border="1" class="titulo">
            <tr>
                ...
                <td colspan="3" style="border-top: 2px solid #ccc;">{{$orden->proveedor()->nombre}}</td>
                <th width="22%" class="center" style="font-size: 8px"><b>FECHA DE EXPEDICIÓN (DD/MM/AA)</b></th>
            </tr>
            <tr>
                <th class="right smalltd">DIRECCIÓN</th>
                <td colspan="3">{{$orden->proveedor()->direccion}}</td>
                <td class="center" style="    border-right: 2px solid #ccc;">{{date('d/m/Y', strtotime($orden->fecha))}}</td>
            </tr>
            <tr>
                <th class="right smalltd">CIUDAD</th>
                <td colspan="3">{{$orden->proveedor()->ciudad}}</td>
                <th class="center" style="font-size: 8px"><b>FECHA DE ENTREGA (DD/MM/AA)</b></th> 
            </tr>
            <tr>
                <th class="right smalltd">TELÉFONO</th>
                <td style="border-bottom: 2px solid #ccc;">{{$orden->proveedor()->telefono1}}</td>
                <th class="right" style="padding-right: 2px;">{{$orden->proveedor()->tip_iden('mini')}}</th>
                <td style="border-bottom: 2px solid #ccc;" >{{$orden->proveedor()->nit}}</td>
                <td class="center" style="border-right: 2px solid #ccc; border-bottom: 2px solid #ccc;">{{date('d/m/Y', strtotime($orden->vencimiento))}}</td>
            </tr>
    ...
    <div style="margin-top: 2%;">
        <table border="0" class="desgloce" >
            ...
            <tfoot>
                <tr class="foot">
                    <th colspan="3" class="smalltd">{{$orden->notas}}</th>
                    <td class="right">SubTotal</td>
                    <td class="right padding-right">{{Auth::user()->empresa()->moneda}}{{App\Funcion::Parsear($orden->total()->subtotal)}}</td>
                </tr>
                <tr class="foot">
                    <td colspan="3" class="smalltd"></td>
                    <td class="right">Descuento</td>
                    <td class="right padding-right">{{Auth::user()->empresa()->moneda}}{{App\Funcion::Parsear($orden->total()->descuento)}} </td>
                </tr>
                @if($orden->total()->descuento>0)
                    
                    <tr class="foot">
                        <td colspan="3" class="smalltd"></td>
                        <td class="right">SubTotal</td>
                        <td class="right padding-right">{{Auth::user()->empresa()->moneda}}{{App\Funcion::Parsear($orden->total()->subsub)}}</td>
                    </tr>
                @endif
                @if($orden->total()->imp)
                ...
                <tr class="foot">
                    ...
                    <th class="right padding-right">{{Auth::user()->empresa()->moneda}}{{App\Funcion::Parsear($orden->total()->total)}} </th>
                </tr>
            </tfoot>
    ...
    <div style="width: 70%; margin-top: 1%">
        <div style="padding-top: 8%; text-align: center;">
            <div style="display: inline-block; width: 45%; border-top: 1px solid #000;     margin-right: 10%;">
                <p class="small"> ELABORADO POR: {{strtoupper($orden->compradorName)}}</p>
            </div>
