...
@section('content')
    ...
    margin-top: 2%;">
            <table class="text-center cajita">
                <tr><td style="padding-top: 5px;padding-bottom: 5px;"><small>NOTA DEBITO</small></td></tr>
    ...
    <div style="">
        <table border="1" class="titulo">
            <tr>
                ...
                <td colspan="3" style="border-top: 2px solid #ccc;">{{$nota->proveedor()->nombre}}</td>
                <th width="22%" class="center" style="font-size: 8px"><b>FECHA DE EXPEDICIÓN (DD/MM/AA)</b></th>
            </tr>
            <tr>
                <th class="right smalltd">DIRECCIÓN</th>
                <td colspan="3">{{$nota->proveedor()->direccion}}</td>
                <td class="center" rowspan="3" style="font-size: 14px;    border-right: 2px solid #ccc;">{{date('d/m/Y', strtotime($nota->fecha))}}</td>
            </tr>
            <tr>
                <th class="right smalltd">CIUDAD</th>
                <td colspan="3">{{$nota->proveedor()->municipio()->nombre}}</td>
            </tr>
            <tr>
                <th class="right smalltd">TELÉFONO</th>
                <td style="border-bottom: 2px solid #ccc;">{{$nota->proveedor()->telefono1}}</td>
                <th class="right smalltd">{{$nota->proveedor()->tip_iden('mini')}}</th>
                <td style="border-bottom: 2px solid #ccc;">{{$nota->proveedor()->nit}}</td>
            </tr>
    ...
    <table border="0" class="desgloce" >
        <thead><tr>
            <th width="30%" style="padding: 3px;">FACTURA DE COMPRA</th>
            <td style="padding: 3px;" class="border_right border_top">{{$nota->factura_detalle()}}</td>
        </tr></thead>
        <tr>
            <th style="padding: 3px;">CONCEPTO NOTA DEBITO</th>
        ...
        <table border="0" class="desgloce" >
            ...
            @if(isset($retenciones))
                @foreach($retenciones as $retencion)
                    <tr class="foot">
                        <td colspan="3" class="smalltd"></td>
                        <td class="right">{{$retencion->nombre}}%</td>
